/**
 * Canonical demo workspace data, shared by the API server seed and the client
 * fallback so the app shows the same content with or without a backend.
 *
 * Dates are generated relative to "today" (daysFromNow) so the demo always
 * looks current. This module is plain data — no runtime dependencies — so it
 * can be bundled by both the esbuild API server and the Vite client.
 */
...
export const project = (
  ...
  packages:
    values.packages ??
    sharedPackages.map((p) => ({ ...p, features: [...p.features] })),
});
